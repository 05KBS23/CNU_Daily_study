...
st.metric(label = "temp", value = '30.5', delta = '2.5℃')
st.metric(label = "temp", value = '30.5', delta = '-2.5℃')
...
edited_titanic = st.experimenral_data_editor(titanic)

if st.button('Press button to Save tatanic2.csv'):   # ***를 원하는 파일명으로 바꾸세요.
    edited_titanic.to_csv('streamlit/tatanic2.csv')
    st.write('💾 Saved')
